 ...
 */
    ...
    public:
         ...
         * 如果需要知道有多少线程是空闲的，应当调用 n_wait_size() 方法。
         */
         ...
         */
        int n_wait_size() { return this->nWaiting; }
         ...
         * 
         * @note
         * 当增加线程数量时，新线程将初始化并待新的任务。当减少线程数量时，超出部分的线程将被通知停止，
         * 一旦它们完成了当前正在执行的任务，就会被销毁。这个方法应该谨慎调用，因为在操作过程中可能会影响正在执行的任务。
         * 特别是，不应该在多个线程中同时调用这个方法，同时也不应该等与 `stop()` 方法交错执行，以免导致不可预料的结果。
         * 
